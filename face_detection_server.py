...
obama_image = face_recognition.load_image_file("/home/jae/PycharmProjects/UDP/registered_face/1.jpg")
obama_face_encoding = face_recognition.face_encodings(obama_image)[0]

# Load a second sample picture and learn how to recognize it.
trump_image = face_recognition.load_image_file("/home/jae/PycharmProjects/UDP/registered_face/trump.jpg")
trump_face_encoding = face_recognition.face_encodings(trump_image)[0]

# Load a second sample picture and learn how to recognize it.
yo_image = face_recognition.load_image_file("/home/jae/PycharmProjects/UDP/registered_face/3.jpg")
yo_face_encoding = face_recognition.face_encodings(yo_image)[0]

# Load a second sample picture and learn how to recognize it.
moon_image = face_recognition.load_image_file("/home/jae/PycharmProjects/UDP/registered_face/irene.jpg")
moon_face_encoding = face_recognition.face_encodings(moon_image)[0]

known_face_encodings = [obama_face_encoding, trump_face_encoding, yo_face_encoding, moon_face_encoding]
known_face_metadata = []
known_face_names = ["gangmin", "intruder", "trump", "trump"]

def restart():
    print("오류로 인해 프로그램 재시작")
...
def save_known_faces():
    with open("known_faces.dat",
              "wb") as face_data_file:  # 파이썬 내장함수 open은 파일이름과 파일 열기모드를 입력값으로 받고 결과값으로 파일 객체를 돌려준다. known_faces라는 dat 파일을 만든다. 이를 face_data_file로 칭한다.
...
def load_known_faces():
    global known_face_encodings, known_face_metadata  # 전역 변수로 known_face_encodings, known_face_metadata를 선언해준다. 리스트인데 어떻게 하지 근데?
...
def lookup_known_face(face_encoding):
    ...
    face_distances = face_recognition.face_distance(known_face_encodings,
                                                    face_encoding)  # face_recognition.face_distance가 가리키는 것이 바로 얼굴 유사성 테스트이다.

    # Get the known face that had the lowest distance (i.e. most similar) from the unknown face.
    best_match_index1 = numpy.argmin(face_distances)  # 리스트에서 비교한 값들 중에 최소값의 위치를 best_match_index로 저장한다.

    # 얼굴 유사성이 0.6 이하이면 얼굴이 맞다고 판단한다.
    # 0.6이라는 값은 얼굴인식모델이 딥러닝 한 값을 나타낸다. 같은 사람일  경우 0.6보다 낮은 값을 나타내었다.
    # 유사성을 낮출수록 얼굴 인식을 더 깐깐하게 할 수 있다.
    if face_distances[best_match_index1] < 0.53:  # 얼굴 유사성이 0.5보다 작을 경우
        # metadata를 known_face_metadata 리스트의 best_match_index 자리의 값으로 둔다.
        metadata = known_face_metadata[best_match_index1]

        # metadata 값에 기존에 저장되어 있던 last_seen과 seen_frames을 업데이트 한다.
        metadata["last_seen"] = datetime.now()
        metadata["seen_frames"] += 1
        metadata["face_dist"] = face_distances[best_match_index1]
...
def main_loop():
    ...
    while True:
        ...
        if length is None:  # 보낸 데이터가 없으면 오류 구문을 나타낸다.
            ...
            # BGR 컬러를 RGB 칼라로 변환시킨다. 이를 rgb_small_frame이라고 칭한다.
            ...
            for face_location, face_encoding in zip(face_locations, face_encodings):
                ...
                # 침입자 감지시 아래의 코드가 실행
                    ...
                    # 인식된 얼굴 전송
                    conn.send('AA'.encode('utf-8'))  # 이부분을 conn으로 변경하였다.
                    time.sleep(0.5)
                    conn.sendall((str(len(img_str))).encode().ljust(16) + img_str)
                    print('침입자 감지 메세지 전송 완료')
                ...
                for (top, right, bottom, left), face_label in zip(face_locations, face_names):
                    ...
                    cv2.putText(frame, name, (left + 4, bottom - 8), cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 255, 255), 1)
                ...
                for metadata in known_face_metadata:
                    ...
                    if datetime.now() - metadata["last_seen"] < timedelta(seconds=10) and metadata["seen_frames"] > 5:
                        ...
                        if visits < 100:
                            ...
                            time_at_door = datetime.now() - metadata['first_seen_this_interaction']
                            # 문을 여는 조건
                            if int(time_at_door.total_seconds()) >= 10:
                                if sim_per >= 70:
                                    conn.send('BB'.encode('utf-8'))  # 이부분을 conn으로 변경하였다.
                                elif sim_per <=45:
